...
def test_classify_entity_cohort_uses_default_florida_windows() -> None:
    reference_date = date(2026, 4, 14)

    assert (
        classify_entity_cohort(
            make_entity("Fresh LLC", last_transaction_date="2026-04-08"),
    ...
    assert (
        classify_entity_cohort(
            make_entity("Tempered LLC", last_transaction_date="2026-03-10"),
            reference_date=reference_date,
        )
    ...
    assert (
        classify_entity_cohort(
            make_entity("Mature LLC", last_transaction_date="2026-01-10"),
            reference_date=reference_date,
        )
        == EntityCohort.mature
    )
